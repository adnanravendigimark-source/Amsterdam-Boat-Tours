...
export default async function Post({ params }: { params: { slug: string } }) {
  ...
  const articleJsonLd = buildArticleJsonLd({
    ...
    authorName: "Amsterdam Boat Tours",
    ...
    <>
      ...
      <main>
        <div className="mx-auto max-w-4xl px-4 pt-8 sm:px-6">
          ...
          <div className="relative mt-8 aspect-[21/9] w-full overflow-hidden rounded-2xl border border-slate-200/80 shadow-md">
            <SafeImage
              src={post.image}
              alt={post.imageAlt}
              fill
        ...
        <div className="mx-auto max-w-6xl px-4 pb-20 pt-12 sm:px-6 lg:grid lg:grid-cols-[1fr_21rem] lg:gap-12">
          <div>
            ...
            <div className="mt-12 rounded-2xl border border-sky-200/80 bg-gradient-to-br from-sky-50/60 via-white to-blue-50/40 p-8 shadow-sm">
              <p className="font-display text-xl font-bold text-slate-900">{post.ctaHeading}</p>
              <p className="mt-2 text-sm leading-relaxed text-slate-600">{post.ctaBody}</p>
              <Link
                href={post.ctaButtonHref}
                className="mt-6 inline-flex rounded-xl bg-gradient-to-r from-blue-600 via-indigo-600 to-sky-600 px-6 py-3 text-sm font-bold text-white shadow-md shadow-blue-600/20 transition hover:scale-[1.02]"
              >
                {post.ctaButtonText}
              </Link>
            </div>
          ...
          <div className="mt-14 lg:mt-0 lg:border-l lg:border-slate-200/80 lg:pl-10">
